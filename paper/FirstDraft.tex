...
Aside from application-aware memory allocation, the attention needs to be drawn also to the efficient management of the on-chip request buffer in order to allow a appropiate ratio of instruction fetch for both compute \todo[inline, size=\tiny]{Shoulf this not be procceors?  Not cores?} cores, without a significant degradation in performance for neither of them. The staged memory scheduler \cite{SmS} proves a good solution for the presented issue without a considerable impact on die area and costs considerations, taking in account..\todo[inline]{Missing stuff from Saevar}
