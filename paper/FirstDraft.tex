...
\begin{abstract}
	Due to several factors, CPU-GPU heterogeneous processors are becoming more and more common. Having both types of processing units on the same design is the cause of several issues that engineers are currently trying to solve. One that has a big impact on performance is the memory hierarchy. Both units have very different and conflicting memory needs that generally have the effect of starving the CPU. In this paper we look mainly at three proposed solutions to this problem. We describe them in detail a long with experimental results, and provide an analysis into the pros and cons of each one. A fourth article was also reviewed that adds some more insight.
\end{abstract}
