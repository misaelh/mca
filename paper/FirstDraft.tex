...
\section{Solutions}
\subsection{Cache line allocation in shared LLC}
The proposed solution is a new cache management policy called HeLM (Heterogeneous LLC Management). It consists on measuring the cache sensitivity of the GPU and CPU, and based on some thresholds decide if a GPU access request to the LLC should be bypassed directly to the main memory or not.
...
\section{Staged Memory Scheduling: Achieving High Performance and Scalability in Heterogeneous Systems}
\subsection{Problem}
When a CPU and GPU are integrated on the same chip and share the same of chip memory the bandwidth demanding GPU can starve the cores of the CPU on lead to a all over poor system performance.\\
Recent memory scheduling has focused on CPU multicore only.  Figure ~\ref{fig:no1}(a)   
...
\centering
	...
	visibility with a larger request buffer .}\label{fig:no1}
\end{figure} 
